    ...
    <div class="pagination">
        <ul>
            ...
            <li
                ...
                @click="select(item)"
            >
                <span class="pages-span">{{ item }}</span>
            </li>
            <!-- 后一页 -->
            <li @click="turnToPage(+1)">
                <img
...
export default {
    ...
    mounted() {
        let queryCurrentPage = document.querySelectorAll("span.pages-span");
        // console.log(this.$refs.pageSpan);
        // console.log(this.$refs.pageSpan[0].innerText);
        ...
        // TODO
        // (1 ~ 4) (totalPages - 4 ~ totalPages) 显示有问题
        for (let i = 0; i < queryCurrentPage.length; i++) {
            // 注意: 使用等于号 == , 不能使用严格等于号 ===
            if (queryCurrentPage[i].innerHTML == this.currentPage) {
                queryCurrentPage[i].style.backgroundColor = "#2d8cf0";
                queryCurrentPage[i].style.boderColor = "#2d8cf0";
            }
        }
    },
    computed: {
